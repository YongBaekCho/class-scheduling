...
def inc_class_plot(inc_classes):
    ...
    new_inc_classes.plot(kind = "bar", rot = 20)
...
def dec_class_plot(dec_classes):

    files = get_frame()
    plt.figure()

    dec_classes = dec_classes.sort_values(ascending=True).head(10)
    course_name, course_num = [], []

    for course in dec_classes.index:
        for i in range(len(files)):
            for row in files[i].index:
                if files[i].loc[row, 'Course ID'] == course:
                    if course == files[i].loc[row, 'Course ID'] and files[i].loc[row,'Subject'] + str(files[i].loc[row,'Catalog']) not in course_name:
    ...
    new_inc_classes = pd.Series(dec_classes.values,course_name[:10])
    new_inc_classes.plot(kind = "bar", rot = 20)

    ax = plt.gca()
    ax.set_xlabel('Name of Class', fontsize = 20)
    ax.set_ylabel('Slope for Class', fontsize = 20)
    ax.set_title("Top 10 Decreasing classes", fontsize = 24)
...
def inc_department_plt(inc_dept):

    files = get_frame()
    plt.figure()

    #inc_dept = inc_dept.sort_values(ascending=False).head(10)
    inc_dept = inc_dept.sort_values(ascending=False)
    department_name = []

    for dept in inc_dept.index:
        for i in range(len(files)):
            for row in files[i].index:
                if files[i].loc[row, 'Subject'] == dept:
                    if dept == files[i].loc[row,'Subject'] and files[i].loc[row,'Subject'] not in department_name:
                        department_name.append(files[i].loc[row,'Subject'])  

    new_inc_dept = pd.Series(inc_dept.values,department_name)
    new_inc_dept.plot(kind = "bar", rot = 90)

    ax = plt.gca()
    ax.set_xlabel('Name of Department', fontsize = 20)
    ax.set_ylabel('Slope for Department', fontsize = 20)
    ax.set_title("Increasing Departments", fontsize = 24)
    pass
def dec_department_plt(dec_dept):

    files = get_frame()
    plt.figure()

    dec_dept = dec_dept.sort_values(ascending=True)
    department_name = []

    for dept in dec_dept.index:
        for i in range(len(files)):
            for row in files[i].index:
                if files[i].loc[row, 'Subject'] == dept:
                    if dept == files[i].loc[row,'Subject'] and files[i].loc[row,'Subject'] not in department_name:
    ...
    new_dec_dept = pd.Series(dec_dept.values,department_name)
    new_dec_dept.plot(kind = "bar", rot = 90)

    ax = plt.gca()
    ax.set_xlabel('Name of Department', fontsize = 20)
    ax.set_ylabel('Slope for Department', fontsize = 20)
    ax.set_title("Decreasing Departments", fontsize = 24)
    pass
def sbs_dict(dict_):

    plt.figure()

    sbs = pd.Series(dict_['COLLEGE OF SOCIAL & BEHAVIORAL SCIENCES'], index = ['Spring14', 'Fall14', 'Spring15', 'Fall15', 'Spring16', 'Fall16', 'Spring17', 'Fall17', 'Spring18', 'Fall18'])

    sbs.plot(kind = 'line')
    plt.xticks(range(10),sbs.index,rotation=20)

    ax = plt.gca()
    ax.set_xlabel('Semesters', fontsize = 15)
    ax.set_ylabel('Mean of total enrollment', fontsize = 15)
    ax.set_title("Mean of total enrollment of College of Social Behavioral and Sciences", fontsize = 15)
    pass
def histo(dictionary):

    plt.figure()

    plt.hist(dictionary.values(), bins = 10, range=[5,45])

    ax = plt.gca()
    ax.set_xlabel('Average enrollment of 10 semesters', fontsize = 20)
    ax.set_ylabel('Number of class', fontsize = 20)
    ax.set_title("Increasing classes' average enrollment", fontsize = 24)
    pass
def eller_dict(dict):
    plt.figure()
    csc = pd.Series(dict['ELLER COLLEGE OF MANAGEMENT'], ['Spring14', 'Fall14', 'Spring15', 'Fall15', 'Spring16', 'Fall16', 'Spring17', 'Fall17', 'Spring18', 'Fall18'])
    csc.plot(kind = 'line')
    plt.xticks(range(10),csc.index,rotation=20)
    
    ax = plt.gca()
    ax.set_xlabel('Semesters', fontsize = 20)
    ax.set_ylabel('Mean enrolment', fontsize = 20)
    ax.set_title("Mean of total enrlloment of Eller College", fontsize = 24)
    pass
...
def main():
    ...
    print(in_dept, de_dept, in_col, de_col, plt_dict)
